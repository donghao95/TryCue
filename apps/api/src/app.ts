...
import { encodeSse, getLatestLiveEventSequence, listLiveEvents, onRunLiveEvent, pushLiveEvent, recordLiveEvent } from "./liveEvents.js";
...
import { createComment, ensurePostState, likeComment, openPost, setPostReaction, sharePost } from "./runtime/interactions.js";
import {
  audienceDetailView,
...
export async function buildApp(config: AppConfig) {
  ...
  app.get("/api/runs/:runId/post-state", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const run = await prisma.testRun.findUnique({ where: { id: runId } });
      if (!run) throw new ApiError("RUN_NOT_FOUND", "试映任务不存在", 404);
      ...
      return ok({ runId, contentVersionId: state.contentVersionId, postState: postStateView(state, viewerState) });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });

  app.get("/api/runs/:runId/comments", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const run = await prisma.testRun.findUnique({ where: { id: runId } });
      if (!run) throw new ApiError("RUN_NOT_FOUND", "试映任务不存在", 404);
      ...
      const page = await listCommentPage(prisma, {
        ...
        limit,
        cursor: query.cursor,
        sort: parseCommentSort(query.sort ?? (query.order === "latest" ? "latest" : query.order))
      });
  ...
  app.post("/api/runs/:runId/post/open", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const result = await prisma.$transaction(async (tx) => {
        ...
        const event = await recordLiveEvent(tx, {
          ...
          payload: { contentVersionId: content.id, simulatedTime: opened.simulatedTime, postState: postStateView(opened.postState) }
        });
        return { event, postState: opened.postState, simulatedTime: opened.simulatedTime };
      });
      pushLiveEvent(runId, result.event);
      return ok({ postState: postStateView(result.postState), simulatedTime: result.simulatedTime });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });

  app.post("/api/runs/:runId/post/like", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const actor = await getDefaultHumanActor(tx);
        const reaction = await setPostReaction(tx, { runId, contentVersionId: content.id, actor, reactionType: "like", active: body.active ?? true });
        ...
        const event = await recordLiveEvent(tx, {
          runId,
          eventType: "post_state.updated",
          payload: { contentVersionId: content.id, simulatedTime: reaction.simulatedTime, postState: postStateView(postState, viewerState) }
        });
        return { event, postState, viewerState, simulatedTime: reaction.simulatedTime, active: reaction.active };
      });
      pushLiveEvent(runId, result.event);
      return ok({ active: result.active, postState: postStateView(result.postState, result.viewerState), simulatedTime: result.simulatedTime });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });

  app.post("/api/runs/:runId/post/favorite", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const body = (request.body ?? {}) as { active?: boolean };
      const result = await prisma.$transaction(async (tx) => {
        ...
        await ensurePostState(tx, runId, content.id);
        const actor = await getDefaultHumanActor(tx);
        const reaction = await setPostReaction(tx, { runId, contentVersionId: content.id, actor, reactionType: "favorite", active: body.active ?? true });
        const postState = await tx.simulatedPostState.findUniqueOrThrow({ where: { contentVersionId: content.id } });
        const viewerState = await getPostViewerState(tx, content.id, actor);
        const event = await recordLiveEvent(tx, {
          runId,
  ...
  app.post("/api/runs/:runId/post/share", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const result = await prisma.$transaction(async (tx) => {
        const content = await requireSingleContentVersion(tx, runId);
        ...
        const event = await recordLiveEvent(tx, {
          runId,
          eventType: "post_state.updated",
          payload: { contentVersionId: content.id, simulatedTime: shared.simulatedTime, postState: postStateView(shared.postState, viewerState) }
        });
        return { event, postState: shared.postState, viewerState, simulatedTime: shared.simulatedTime };
      });
      pushLiveEvent(runId, result.event);
      return ok({ postState: postStateView(result.postState, result.viewerState), simulatedTime: result.simulatedTime });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });

  app.post("/api/runs/:runId/comments", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      ...
      if (contentText.length > MAX_COMMENT_LENGTH) throw new ApiError("VALIDATION_ERROR", `评论内容不能超过 ${MAX_COMMENT_LENGTH} 字`, 400);
      const result = await prisma.$transaction(async (tx) => {
        const content = await requireSingleContentVersion(tx, runId);
        await ensurePostState(tx, runId, content.id);
        const actor = await getDefaultHumanActor(tx);
        const created = await createComment(tx, { runId, contentVersionId: content.id, actor, content: contentText, parentCommentId: body.parentCommentId ?? body.replyToCommentId });
        const commentEvent = await recordLiveEvent(tx, {
          runId,
          eventType: "comment.created",
          payload: { contentVersionId: content.id, simulatedTime: created.simulatedTime, comment: commentView(created.comment) }
        });
        ...
        const postEvent = await recordLiveEvent(tx, {
          runId,
          eventType: "post_state.updated",
          payload: { contentVersionId: content.id, simulatedTime: created.simulatedTime, postState: postStateView(created.postState) }
        });
        return { commentEvent, parentCommentEvent, postEvent, comment: created.comment, postState: created.postState, simulatedTime: created.simulatedTime };
      });
      pushLiveEvent(runId, result.commentEvent);
      if (result.parentCommentEvent) pushLiveEvent(runId, result.parentCommentEvent);
      pushLiveEvent(runId, result.postEvent);
      return ok({ comment: commentView(result.comment), postState: postStateView(result.postState), simulatedTime: result.simulatedTime });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });

  app.post("/api/runs/:runId/comments/:commentId/replies", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        await ensurePostState(tx, runId, content.id);
        const actor = await getDefaultHumanActor(tx);
        const created = await createComment(tx, { runId, contentVersionId: content.id, actor, content: contentText, parentCommentId: commentId });
        const commentEvent = await recordLiveEvent(tx, {
          runId,
          eventType: "comment.created",
          payload: { contentVersionId: content.id, simulatedTime: created.simulatedTime, comment: commentView(created.comment) }
  ...
  app.post("/api/runs/:runId/comments/:commentId/like", async (request, reply) => {
    try {
      const runId = getRunId(request.params);
      const commentId = (request.params as { commentId?: string }).commentId;
      if (!commentId) throw new ApiError("VALIDATION_ERROR", "评论 ID 缺失", 400);
      ...
      const result = await prisma.$transaction(async (tx) => {
        const content = await requireSingleContentVersion(tx, runId);
        const actor = await getDefaultHumanActor(tx);
        const liked = await likeComment(tx, { runId, contentVersionId: content.id, actor, commentId, active: body.active ?? true });
      ...
      pushLiveEvent(runId, result.event);
      return ok({ comment: result.comment, simulatedTime: result.simulatedTime });
    } catch (error) {
      return sendApiError(reply, error);
    }
  });
...
function parsePageQuery(query: unknown, defaultLimit = 10) {
